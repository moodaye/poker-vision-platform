...
def _raw(predictions: list[dict], img_w: int = 1920, img_h: int = 1080) -> dict:
    """Build a minimal Roboflow raw-prediction dict."""
    return {"image": {"width": img_w, "height": img_h}, "predictions": predictions}


def _pred(cls: str, x: float, y: float, w: float, h: float, conf: float = 0.95) -> dict:
...
class TestConfidenceFiltering:
    ...
    def test_skip_classes_excluded(self):
...
class TestYoloCoordinates:
    def test_output_values_normalised(self):
    ...
    def test_correct_class_id_assigned(self):
...
class TestTemplateInjection:
    ...
    def test_template_injected_when_class_not_detected(self):
    ...
    def test_template_not_injected_when_already_detected(self):
    ...
    def test_template_not_injected_without_poker_table(self):
    ...
    def test_template_coordinates_clamped(self):
...
class TestWriteDataYaml:
    ...
    def test_names_is_list_in_yaml(self, tmp_path: Path):
    ...
    def test_names_order_preserved(self, tmp_path: Path):
        write_data_yaml(tmp_path, self.CLASS_NAMES)
        with open(tmp_path / "data.yaml") as f:
            content = yaml.safe_load(f)
        assert content["names"] == self.CLASS_NAMES

    def test_names_count_matches(self, tmp_path: Path):
        write_data_yaml(tmp_path, self.CLASS_NAMES)
        with open(tmp_path / "data.yaml") as f:
            content = yaml.safe_load(f)
