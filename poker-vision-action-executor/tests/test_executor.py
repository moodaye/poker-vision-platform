...
def _patch_executor(
    ...
    button_rect: tuple = (100, 200, 200, 230),
    ...
    return (
        ...
        patch("executor.list_child_buttons", return_value=[(55, "Fold"), (56, "Call 50"), (57, "Check"), (58, "Raise To")]),
...
def test_watching_returns_success_without_window_search():
...
def test_watching_case_insensitive():
    from executor import execute
...
def test_raise_without_amount_returns_failure():
    from executor import execute
...
def test_bet_without_amount_returns_failure():
    from executor import execute
...
def test_raise_with_amount_does_not_fail_on_missing_amount_check():
    from executor import execute
...
def test_fold_fails_when_no_window_found():
    from executor import execute
...
def test_fold_uses_configured_title_hints_by_default():
    from executor import execute, _WINDOW_TITLE_HINTS
...
def test_window_title_hint_overrides_config():
    from executor import execute
...
def test_fold_fails_when_no_button_found():
    from executor import execute

    with patch("executor.find_poker_window", return_value=99), \
         patch("executor.list_child_buttons", return_value=[(10, "Call"), (11, "Raise")]):
        result = execute("fold")
...
def test_fold_success():
    ...
    patches = _patch_executor()
    with patches[0], patches[1], patches[2], patches[3], patches[4] as mock_click, patches[5], patches[6]:
        result = execute("fold")
...
def test_call_matches_dynamic_label():
...
def test_raise_calls_enter_bet_amount_then_clicks():
    from executor import execute

    patches = _patch_executor()
    with patches[0], patches[1], patches[2], patches[3], patches[4] as mock_click, \
         patches[5] as mock_bet, patches[6]:
        result = execute("raise", amount=300)
...
def test_raise_fails_when_bet_entry_not_found():
    from executor import execute
...
def test_dry_run_does_not_enter_bet_amount():
    from executor import execute
...
def test_dry_run_includes_screen_position_in_message():
    from executor import execute
