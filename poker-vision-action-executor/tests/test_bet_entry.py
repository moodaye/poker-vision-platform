...
def test_is_numeric_text_empty_string():
    from bet_entry import _is_numeric_text
    assert _is_numeric_text("") is True


def test_is_numeric_text_digits_only():
    from bet_entry import _is_numeric_text
    assert _is_numeric_text("300") is True


def test_is_numeric_text_with_comma_separator():
    from bet_entry import _is_numeric_text
    assert _is_numeric_text("1,500") is True


def test_is_numeric_text_with_decimal():
    from bet_entry import _is_numeric_text
    assert _is_numeric_text("12.5") is True


def test_is_numeric_text_rejects_alpha():
    from bet_entry import _is_numeric_text
    assert _is_numeric_text("Call") is False


def test_is_numeric_text_rejects_mixed():
    from bet_entry import _is_numeric_text
...
def test_find_bet_input_returns_first_numeric_edit():
    from bet_entry import find_bet_input

    with patch("bet_entry.list_child_edits", return_value=[(10, "0"), (20, "chat box")]):
        result = find_bet_input(99)

    assert result == 10


def test_find_bet_input_skips_non_numeric_edits():
    from bet_entry import find_bet_input

    with patch("bet_entry.list_child_edits", return_value=[(10, "Enter chat"), (20, "300")]):
        result = find_bet_input(99)

    assert result == 20


def test_find_bet_input_returns_none_when_no_numeric_edit():
    from bet_entry import find_bet_input
...
def test_find_bet_input_returns_none_when_no_edits():
    from bet_entry import find_bet_input
...
def test_enter_bet_amount_raises_on_zero():
...
def test_enter_bet_amount_raises_on_negative():
    from bet_entry import enter_bet_amount
...
def test_enter_bet_amount_returns_false_when_no_edit():
    from bet_entry import enter_bet_amount
...
def test_enter_bet_amount_types_correct_value():
    """Should focus, click, Ctrl+A, then write the amount string."""
    from bet_entry import enter_bet_amount

    with patch("bet_entry.find_bet_input", return_value=55), \
...
def test_enter_bet_amount_correct_screen_centre():
