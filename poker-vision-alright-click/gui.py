...
class OKButtonDetectorGUI:
    ...
    def _create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="OK Button Auto-Clicker", 
                               font=("Arial", 16, "bold"))
        ...
        ttk.Label(controls_frame, text="Confidence Threshold:").grid(row=0, column=0, sticky=tk.W)
        self.confidence_var = tk.DoubleVar(value=config.DEFAULT_CONFIDENCE_THRESHOLD)
        confidence_scale = ttk.Scale(controls_frame, from_=config.MIN_CONFIDENCE_THRESHOLD, 
        ...
        self.confidence_label = ttk.Label(controls_frame, 
        ...
        # Start/Stop button
        self.start_stop_button = ttk.Button(button_frame, text="Start Detection", 
                                          command=self._toggle_detection)
        self.start_stop_button.pack(side=tk.LEFT, padx=(0, 10))
        
        # Test detection button
        self.test_button = ttk.Button(button_frame, text="Test Detection", 
            ...
            try:
                ...
                if detections:
                    ...
                    if self.mouse_controller.click_button(x, y):
                        ...
                        self._log_message(f"Clicked OK button at ({x}, {y}) with confidence {confidence:.3f}")
                    else:
        ...
        try:
            ...
            if detections:
                ...
                for i, (x, y, confidence) in enumerate(detections[:5]):  # Show max 5
                    self._log_message(f"  {i+1}. Position: ({x}, {y}), Confidence: {confidence:.3f}")
            else:
    ...
    def _update_detection_count(self):
        """Update the detection count in the GUI."""
        self.root.after(0, lambda: self.detection_count_label.config(text=str(self.detection_count)))
    
    def _update_click_count(self):
        """Update the click count in the GUI."""
        self.root.after(0, lambda: self.click_count_label.config(text=str(self.click_count)))
